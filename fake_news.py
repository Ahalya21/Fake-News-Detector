...
# ── Step 1.1: Fix encoding issues (curly quotes) 
...
# ── Step 1.3: Handle nulls (FIXED) 
df['title'] = df['title'].replace('', pd.NA)
df['title'] = df['title'].fillna('')

df = df.dropna(subset=['text', 'label'])

print(f"\nAfter null handling: {df.shape}")

# ── Step 1.4: Merge title + text 
...
# ── Step 1.6: Stratified sampling (FIXED PROPERLY) 

target_size = min(70000, len(df))

df_70k = (
    df.groupby('label', group_keys=False)
      .apply(lambda grp: grp.sample(
          n=round(target_size * len(grp) / len(df)),
          random_state=42
      ))
      .reset_index(drop=True)   # IMPORTANT
)

# Ensure label column exists (safety fix)
df_70k = df_70k.copy()

# ── Step 1.7: Save outputs 
df_70k.to_csv('stage1_output.csv', index=False)
df_held_out = df.drop(df_70k.index, errors='ignore').reset_index(drop=True)

df_held_out.to_csv('held_out_test.csv', index=False)

print("\nStage 1 complete.")
print(f"Training data  : stage1_output.csv  → {df_70k.shape}")
